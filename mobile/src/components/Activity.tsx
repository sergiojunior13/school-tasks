...
export function Subject({ children }: ActivityProps) {
  return (
    <Text className="px-3 py-1 font-sans bg-emerald-600 rounded-full justify-center items-center text-zinc-50">
      {children}
    </Text>
  );
}
...
export function Button({
  ...
  return (
    <TouchableOpacity
      activeOpacity={0.7}
      className={
        "items-center justify-center w-9 h-9 rounded-full " + className
      }
      {...rest}
    >
      {children}
...
export function Footer({ children }: ActivityProps) {
  return (
    <View className="justify-between items-center flex-row">{children}</View>
  );
}
...
export function Participants({ children }: ActivityProps) {
  return (
    <View className="flex-row space-x-2">
      ...
      <Text className="text-zinc-400 font-sans">{children}</Text>
    </View>
  );
}
